...
$(document).ready(function() {
	...
	// Execute Action
	$('a.ExecAction.email').click(CRM.OpenEmailAction);
	...
	// Datatables
	//----------------------------------------
	if (document.getElementById('ContactsDT') != null){

		// Store it for easy access later
		CRM.ContactsDT = jQuery('#ContactsDT');

		CRM.CheckboxTimeout = 0;
		CRM.ContactsDT.delegate('tbody tr', 'click', CRM.ToggleExecActionVis);
	}
...
CRM.ToggleExecActionVis = function(Event){
	...
	CRM.CheckboxTimeout = setTimeout(function(){
		if (CRM.ContactsDT.find('tbody').find('tr.Checked').length > 0){
	...
	var Data = CRM.Datatables[ CRM.ContactsDT.data('name') ].fnGetData(Index);
