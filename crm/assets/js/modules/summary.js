...
$(document).ready(function() {
	...
	// Datatables
	//----------------------------------------
	if (document.getElementById('summaryDT') != null){

		// Store it for easy access later
		CRM.summaryDT = jQuery('#summaryDT');

		CRM.CheckboxTimeout = 0;
		CRM.summaryDT.delegate('tbody tr', 'click', CRM.ToggleExecActionVis);
	}
...
CRM.ToggleExecActionVis = function(Event){
	...
	CRM.CheckboxTimeout = setTimeout(function(){
		if (CRM.summaryDT.find('tbody').find('tr.Checked').length > 0){
	...
	var Data = CRM.Datatables[ CRM.summaryDT.data('name') ].fnGetData(Index);
...
CRM.GenerateExcel = function(Event){
	...
	// Send People away!
	window.location.href = CRM.BASE + 'hrm/ajax_generate_excel_summary?ids=' + Contacts.join('|');// +'&conditions=' + Conditions.join('|') ;
};
...
CRM.GeneratePdf = function(Event){
	...
	// Send People away!
	//window.location.href = CRM.BASE + 'hrm/ajax_generate_pdf?ids=' + Contacts.join('|');// +'&conditions=' + Conditions.join('|') ;
	$('#content1').css('display', 'block');
	$('#PDFLOADING').css('display', 'block');
	
	var myPDF = new PDFObject({
		url: CRM.BASE + 'hrm/pdf_report_hrm_summary?ids=' + Contacts.join('|'),
