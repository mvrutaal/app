...
$(document).ready(function() {
	...
	// Datatables
	//----------------------------------------
	if (document.getElementById('HrmDT') != null){

		// Store it for easy access later
		CRM.HrmDT = jQuery('#HrmDT');

		CRM.CheckboxTimeout = 0;
		CRM.HrmDT.delegate('tbody tr', 'click', CRM.ToggleExecActionVis);
	}
...
CRM.ToggleExecActionVis = function(Event){
	...
	CRM.CheckboxTimeout = setTimeout(function(){
		if (CRM.HrmDT.find('tbody').find('tr.Checked').length > 0){
	...
	var Data = CRM.Datatables[ CRM.HrmDT.data('name') ].fnGetData(Index);
...
CRM.GenerateExcel = function(Event){
	...
	// Send People away!
	window.location.href = CRM.BASE + 'hrm/ajax_generate_excel?ids=' + Contacts.join('|');// +'&conditions=' + Conditions.join('|') ;
};
...
CRM.GeneratePdf = function(Event){
	...
	var myPDF = new PDFObject({
		url: CRM.BASE + 'hrm/pdf_report_hrm?ids=' + Contacts.join('|'),
