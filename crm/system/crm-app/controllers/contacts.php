...
class Contacts extends CRM_Controller
{
	...
	public function index()
	{
		...
		// Do we have access?
		//----------------------------------------
		if (!$this->acl->can_read($this->session->userdata['group'], 'contacts')) {
		...
		$data['standard_cols']['street']		= array('name' => 'Street', 'sortable' => 'true');
		$data['standard_cols']['tel_work']		= array('name' => 'Tel. Work', 'sortable' => 'false');
		$data['standard_cols']['tel_mobile']	= array('name' => 'Tel. Mobile', 'sortable' => 'false');
		$data['standard_cols']['email_work']	= array('name' => 'Email Work', 'sortable' => 'false');
		$data['standard_cols']['job_title']		= array('name' => 'Job Title', 'sortable' => 'true');
		$data['standard_cols']['company']		= array('name' => 'Company', 'sortable' => 'true');

		$data['extra_cols']['nickname']	= array('name' => 'Nickname', 'sortable' => 'false');
		$data['extra_cols']['sex']		= array('name' => 'Sex', 'sortable' => 'false');
		$data['extra_cols']['birthday']	= array('name' => 'Birthday', 'sortable' => 'false');
		...
		$data['extra_cols']['country']	= array('name' => 'Country', 'sortable' => 'false');
		$data['extra_cols']['tel_home']	= array('name' => 'Tel. Home', 'sortable' => 'false');
		$data['extra_cols']['tel_mobile2']		= array('name' => 'Tel. Mobile 2', 'sortable' => 'false');
		$data['extra_cols']['email_work2']		= array('name' => 'Email Work 2', 'sortable' => 'false');
		$data['extra_cols']['email_personal']	= array('name' => 'Email Personal', 'sortable' => 'false');
		$data['extra_cols']['email_personal2']	= array('name' => 'Email Personal 2', 'sortable' => 'false');
		$data['extra_cols']['id_number']		= array('name' => 'ID Number', 'sortable' => 'false');
		$data['extra_cols']['bankaccount_number'] = array('name' => 'Bank Account #', 'sortable' => 'false');

		//----------------------------------------
		// Grab all companies
		//----------------------------------------
		$data['companies'] = array('0' => 'NONE');
		$query = $this->db->query("SELECT * FROM crm_companies ORDER BY company_title ASC");
		...
		//----------------------------------------
		$data['groups'] = array();
		$query = $this->groups_model->get_groups('contacts');

		foreach($query as $row)
		{
			$data['groups'][$row->group_id] = $row->group_name;
		}

		$vData = array();
		$vData['title'] = 'Contacts';
		$vData['pagetype'] = 'contacts';
		$vData['content'] = $this->load->view('contacts/index', $data, TRUE);
	...
	public function add($contact_id=0)
	{
		...
		// Do we have access?
		//----------------------------------------
		if (! $this->acl->can_write($this->session->userdata['group'], 'contacts'))
		{
		...
		else
		{
			...
			//----------------------------------------
			// Grab all linked groups
			//----------------------------------------
			$data['groups'] = $this->groups_model->get_linked_groups($contact_id);
		}
		...
		// Grab all groups
		//----------------------------------------
		$data['dbgroups'] = array('0' => '');
		$query = $this->groups_model->get_groups('contacts');

		foreach($query as $row)
		{
			$data['dbgroups'][$row->group_id] = $row->group_name;
		}
		...
		//----------------------------------------
		$vData = array();
		$vData['title'] = ($contact_id == 0) ? 'New Contact' : 'Edit Contact';
		$vData['pagetype'] = 'contacts';
		$vData['content'] = $this->load->view('contacts/add_edit', $data, TRUE);
		$this->load->view('layout', $vData);
	}
	...
	public function ajax_datatable()
	{
		...
		$sStreet = ($this->input->post('street')  != FALSE) ? $_POST['street'] : FALSE;
		...
		$query = $this->db->select('COUNT(*) as total_records', FALSE)->from('crm_contacts')->get();
		...
		// Real Query
		//----------------------------------------
		$this->db->select('c.*, cp.company_title, sub.suburb_label, str.street_label, city.city_label, cc.country_label');
		$this->db->from('crm_contacts c');
		...
		$this->db->join('crm_dataset_cities city', 'city.city_id = c.city_id', 'left');
		$this->db->join('crm_dataset_countries cc', 'cc.country_id = c.country_id', 'left');
		...
		for ($i = 0; $i < $sort_cols; $i++)
		{
			...
			switch ($col)
			{
				...
				case 'last_name':
					...
					break;
				case 'street':
					$this->db->order_by('c.street', $sort);
					break;
				case 'job_title':
					$this->db->order_by('c.job_title', $sort);
					break;
				case 'company':
					$this->db->order_by('cp.company_title', $sort);
					break;
				default:
		...
		foreach ($query->result() as $row)
		{
			...
			// Actions Block
			$actions = '<a href="'.site_url('contacts/view/'.$row->contact_id).'" class="view">'.$row->contact_id.'</a>';
			if ($can_edit) $actions .= '<a href="'.site_url('contacts/edit/'.$row->contact_id).'" class="edit"></a>';
