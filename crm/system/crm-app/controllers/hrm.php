...
class Hrm extends CRM_Controller
{
	...
	public function index()
	{
		...
		}else if($this->session->userdata['group'] == 'manager'){
			$sqlQuery = "SELECT crm_hrm.*, crm_contacts.first_name, crm_contacts.last_name FROM crm_hrm JOIN crm_contacts ON crm_hrm.contact_id = crm_contacts.contact_id WHERE crm_hrm.created_date LIKE '%$currentYear%' AND crm_hrm.contact_id = ".$sessionUserId;
		}
	...
	// ********************************************************************************* //
	public function edit_leave($rowID){
		
		if (! $this->acl->can_modify($this->session->userdata['group'], 'hrm'))
		{
			show_error('Access Denied!');
		}
		if($_POST){
			
			//Query to get last updated resutls from crm_hrm table
			$Prequery = $this->db->query("SELECT crm_hrm_details.*, crm_hrm.*
								   FROM crm_hrm_details 
							       INNER JOIN crm_hrm ON crm_hrm.contact_id = crm_hrm_details.contact_id
		                           WHERE hrm_id =".$rowID
								  );
			
			$preQueryResults = $Prequery->result();

			$contact_id = $this->input->post('contact_id');
			$holiday_type = $this->input->post('holiday_type');
			$holiday_taken = $this->input->post('holiday_hours');
			$attachment = $this->input->post('attachment');
			$userfile  = $_FILES['userfile'];
			$from_date = $this->input->post('from_date');
			$to_date = $this->input->post('to_date');
			$old_attachment = $this->input->post('oldAttachment');
			
			if($userfile['name']!= ""){
				$upload_path = realpath(APPPATH.'\\uploads\\');
				 if (file_exists($upload_path.$old_attachment)) {
				...
				$this->db->set('attachment', $old_attachment);
			}
			...
			$this->db->set('contact_id', $contact_id);
			$this->db->set('holiday_type', $holiday_type);
			$this->db->set('holiday_taken', $holiday_taken);
			$this->db->set('from_date', $from_date);
			$this->db->set('to_date', $to_date);
			$this->db->set('created_date', $date_str);
			$this->db->where('hrm_id', $rowID);
			$this->db->update('crm_hrm_details');
			
			//Update the crm_hrm table for balance leave
			
			//echo "<pre>";
			//print_r($preQueryResults);die;
			
			foreach($preQueryResults as $row)
			{
				$preHolidayType = $row->holiday_type;
				$currentType = $holiday_type;
				
				$id = $row->id;
				if($preHolidayType == $currentType){
					if ($currentType==2){
						$preHolidayTaken = $row->holiday_taken;
						$preHolidayHours = $row->holidayHours;
						$balanceHours = $row->balanceHours;
						
						$totalHolidayHours = (($preHolidayHours)-($preHolidayTaken)); //update to datatable//
						$totalBalance = (($balanceHours)+($preHolidayTaken)); //update to datatable//
						
						$totalHolidayHours = (($totalHolidayHours)+($holiday_taken));
						$totalBalance = (($totalBalance)-($holiday_taken));
						//write update query here//
						$this->db->set('holidayHours', $totalHolidayHours);
						$this->db->set('balanceHours', $totalBalance);
						$this->db->where('id', $id);
						$this->db->update('crm_hrm');
						
					}else if ($currentType==1){
						$preHolidayTaken = $row->holiday_taken;
						$preSickHours = $row->sickHours;
						$balanceHours = $row->balanceHours;
						
						$totalSickHours = (($preSickHours)-($preHolidayTaken)); //update to datatable//
						$totalBalance = (($balanceHours)+($preHolidayTaken)); //update to datatable//

						$totalSickHours = (($totalSickHours)+($holiday_taken));
						$totalBalance = (($totalBalance)-($holiday_taken));
						//write update query here//
						$this->db->set('sickHours', $totalSickHours);
						$this->db->set('balanceHours', $totalBalance);
						$this->db->where('id', $id);
						$this->db->update('crm_hrm');
					...
					if ($currentType==1){
						...
						$totalSickHours = (($preSickHours)+($holiday_taken));
						$totalBalance = (($totalBalance)-($holiday_taken));
						//write update query here//
						$this->db->set('sickHours', $totalSickHours);
						$this->db->set('holidayHours', $totalHolidayHours);
						...
						$totalHolidayHours = (($preHolidayHours)+($holiday_taken));
						$totalBalance = (($totalBalance)-($holiday_taken));
						//write update query here//
						$this->db->set('sickHours', $totalSickHours);
						$this->db->set('holidayHours', $totalHolidayHours);
			...
			redirect('/hrm/summary/'.$contact_id);

		}
		$query = $this->db->query("SELECT crm_hrm_details.*, crm_contacts.last_name, crm_contacts.first_name 
								   FROM crm_hrm_details 
							       INNER JOIN crm_contacts ON crm_contacts.contact_id = crm_hrm_details.contact_id
		                           WHERE hrm_id =".$rowID
								  );
		...
		foreach($query->result() as $row)
		{
			...
			$data['hrm_id'] = $row->hrm_id;
			$data['contact_id'] = $row->contact_id;
			$data['name'] = $name;
			$data['holiday_type'] = $row->holiday_type;
			$data['holiday_taken'] = $row->holiday_taken;
		...
		$vData = array();
		$vData['title'] = 'Edit Entry';
		$vData['pagetype'] = 'hrm';
		$vData['content'] = $this->load->view('hrm/edit_leave', $data, TRUE);
		$this->load->view('layout', $vData);
	...
	public function ajax_datatable()
	{
		...
		if($this->session->userdata['group'] == 'manager'){
			$this->db->where('HR.contact_id', $sessionUserId);
		}
		...
		*/
		...
		foreach ($query->result() as $row)
		{
			...
			// Actions Block
			//$actions = '<a href="'.site_url('hrm/view/'.$row->contact_id).'" class="view">'.$row->contact_id.'</a>';
	...
	public function ajax_leavedatatable($contact_id)
	{
		...
		*/
		...
		foreach ($query->result() as $row)
		{
			...
			if ($can_edit) $actions .= '<a href="'.site_url('hrm/edit_leave/'.$row->hrm_id).'" class="edit"></a>';
	...
	public function ajax_generate_excel()
	{
		...
		$this->db->select('c.*, da.* ');
		$this->db->from('crm_contacts c');
		$this->db->join('crm_hrm da', 'da.contact_id = c.contact_id', 'inner');
		...
		foreach ($query->result() as $contact)
		{
			...
			//$date_in_print = date("d-m-Y",strtotime($contact->sickHours ));
			//$date_out_print = date("d-m-Y",strtotime($contact->holidayHours ));

			$sickHours = $contact->sickHours;
			$holidayHours = $contact->holidayHours;
			$balanceHours = $contact->balanceHours;
			$initialHours = $contact->initialHours;
			...
			$EmpDetails[$contact->contact_id][$i]['sick_hours'] = $sickHours;
			$EmpDetails[$contact->contact_id][$i]['holiday_hours'] = $holidayHours;
			$EmpDetails[$contact->contact_id][$i]['balanceHours'] = $balanceHours;
			$EmpDetails[$contact->contact_id][$i]['initialHours'] = $initialHours;
		...
		$sheetname = 'Hrm_Report_'.date('d-m-Y');
		...
		$this->excel->getActiveSheet()->setCellValue('C1', 'sickHours');
		$this->excel->getActiveSheet()->setCellValue('D1', 'holidayHours');
		$this->excel->getActiveSheet()->setCellValue('E1', 'balanceHours');
		$this->excel->getActiveSheet()->setCellValue('F1', 'initialHours');
		...
		foreach($EmpDetails as $_EmpDetails):
			...
			foreach($_EmpDetails as $__EmpDetails):
				...
				$this->excel->getActiveSheet()->setCellValue('C'.$j, $__EmpDetails['sick_hours']);
				$this->excel->getActiveSheet()->setCellValue('D'.$j, $__EmpDetails['holiday_hours']);
				$this->excel->getActiveSheet()->setCellValue('E'.$j, $__EmpDetails['balanceHours']);
				$this->excel->getActiveSheet()->setCellValue('F'.$j, $__EmpDetails['initialHours']);
	...
	public function pdf_report_hrm()
	{
		if ($this->input->get('ids') == FALSE) show_error('No Contact ID\'s!!');
		...
		foreach ($query->result() as $contact)
		{
			...
			$EmpDetails[$contact->contact_id][$i]['sick_hours'] = $sickHours;
			$EmpDetails[$contact->contact_id][$i]['holiday_hours'] = $holidayHours;
			$EmpDetails[$contact->contact_id][$i]['balance_hours'] = $balanceHours;
			$EmpDetails[$contact->contact_id][$i]['initial_hours'] = $initialHours;
			//$EmpDetails[$contact->contact_id][$i]['time_spent'] = $spent_time;
		...
		$html .= '
			<h4 style="color:darkgreen;">'.strtoupper('hrm report').'</h4>
		...
		foreach($EmpDetails as $_EmpDetails):
			...
			foreach($_EmpDetails as $__EmpDetails):
				$html .= 		'<tr class="ItemRow">
									...
									<td>'.$__EmpDetails['sick_hours'].'</td>
									<td>'.$__EmpDetails['holiday_hours'].'</td>
									<td>'.$__EmpDetails['balance_hours'].'</td>
									<td>'.$__EmpDetails['initial_hours'].'</td>
								  </tr>
								';
	...
	public function ajax_generate_excel_summary()
	{
		if ($this->input->get('ids') == FALSE) show_error('No Contact ID\'s!!');
		...
		$this->db->select('c.*, da.* ');
		$this->db->from('crm_contacts c');
		$this->db->join('crm_hrm_details da', 'da.contact_id = c.contact_id', 'inner');
		
		//$this->db->where_in('da.contact_id', explode('|',$this->input->get('ids')));
		$this->db->where_in('da.hrm_id', explode('|',$this->input->get('ids')));
		//$this->db->order_by('da.first_access_type');
		//$this->db->group_by('da.contact_id');
		$query = $this->db->get();
		...
		foreach ($query->result() as $contact)
		{
			...
			$holiday_type = $contact->holiday_type;
			$holiday_taken = $contact->holiday_taken;
			$from_date = $contact->from_date;
			$to_date = $contact->to_date;
			...
			$EmpDetails[$contact->contact_id][$i]['contact_id'] = $contact->contact_id;
			$EmpDetails[$contact->contact_id][$i]['name'] = $contact->first_name." ".$contact->last_name;
			$EmpDetails[$contact->contact_id][$i]['holiday_type'] = $holiday_type;
			$EmpDetails[$contact->contact_id][$i]['holiday_taken'] = $holiday_taken;
			$EmpDetails[$contact->contact_id][$i]['from_date'] = $from_date;
			$EmpDetails[$contact->contact_id][$i]['to_date'] = $to_date;
			//$EmpDetails[$contact->contact_id][$i]['time_spent'] = $spent_time;
		...
		$this->excel->getActiveSheet()->setCellValue('A1', 'Contact ID');
		$this->excel->getActiveSheet()->setCellValue('B1', 'Contact Name');
		$this->excel->getActiveSheet()->setCellValue('C1', 'Holiday Type');
		$this->excel->getActiveSheet()->setCellValue('D1', 'Holiday Taken');
		$this->excel->getActiveSheet()->setCellValue('E1', 'From Date');
		$this->excel->getActiveSheet()->setCellValue('F1', 'To Date');
		//$this->excel->getActiveSheet()->setCellValue('G1', 'Hours Spent Inside');
		...
		foreach($EmpDetails as $_EmpDetails):
			...
			foreach($_EmpDetails as $__EmpDetails):
				...
				$this->excel->getActiveSheet()->setCellValue('A'.$j, $__EmpDetails['contact_id']);
				$this->excel->getActiveSheet()->setCellValue('B'.$j, $__EmpDetails['name']);
				$this->excel->getActiveSheet()->setCellValue('C'.$j, $__EmpDetails['holiday_type']);
				$this->excel->getActiveSheet()->setCellValue('D'.$j, $__EmpDetails['holiday_taken']);
				$this->excel->getActiveSheet()->setCellValue('E'.$j, $__EmpDetails['from_date']);
				$this->excel->getActiveSheet()->setCellValue('F'.$j, $__EmpDetails['to_date']);
				//$this->excel->getActiveSheet()->setCellValue('G'.$j, $__EmpDetails['time_spent']);
	...
	public function pdf_report_hrm_summary()
	{
		if ($this->input->get('ids') == FALSE) show_error('No Contact ID\'s!!');
		...
		foreach($EmpDetails as $_EmpDetails):
			...
			foreach($_EmpDetails as $__EmpDetails):
				$html .= 		'<tr class="ItemRow">
									<td>'.$__EmpDetails['contact_id'].'</td>
									<td>'.$__EmpDetails['name'].'</td>
									<td>'.$__EmpDetails['holiday_type'].'</td>
									<td>'.$__EmpDetails['holiday_taken'].'</td>
									<td>'.$__EmpDetails['from_date'].'</td>
									<td>'.$__EmpDetails['to_date'].'</td>
								  </tr>
								';
