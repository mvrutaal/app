...
\*======================================================================*/
	...
	$document->addStyleSheet(JURI::base() . 'modules/mod_yjis3/css/mod_yjis3.css');
	if(preg_match("/msie 6/", $who)) {
	$document->addStyleSheet(JURI::base() . 'modules/mod_yjis3/css/ifie.php');
	}
	$document->addScript(JURI::base() . 'modules/mod_yjis3/src/mod_yjis3.js');
...
foreach ($slides as $slide){
 if(($slide['mbox'])==1){
$document->addScript(JURI::base() . 'modules/mod_yjis3/src/yjmmbox.js'); 
$document->addStyleSheet(JURI::base() . 'modules/mod_yjis3/css/yjmmbox.css');
if(preg_match("/msie 6/", $who)) {
$document->addScript(JURI::base() . 'modules/mod_yjis3/src/ie6minmax.js'); 
}
}
}
  	/**
	...
	<?php foreach ($slides as $slide):?>
		...
		<?php if($slide['link'] || $slide['mbox']):?>
           <?php if(($slide['mbox'])==1){?>
			...
			<a href="<?php echo $slide['mboxlink']?>"  <?php if(($slide['mboxtype'])==1){?>class="popboxv" rel="mediabox[<?php echo $slide['video_width'];?> <?php echo $slide['video_height'];?>]"<?php }else{ ?> class="popbox" <?php if(($slide['image_group'])==1){?>rel="<?php echo $slide['image_group_name'];?>"<?php } ?><?php } ?> title="<?php echo $slide['description'];?>">
            <?php }else{ ?>
...
<div class="navContainer" style="width:<?php echo $swidth;?>px;">
	...
	<a href="#" class="YJS_navLink<?php echo $is_copy?>" title="Navigate to slide <?php echo $key+1;?>::<?php echo $slide['title'];?>"><?php echo $key+1;?></a>	
	<?php endforeach;?>
</div>
<?php endif;?>