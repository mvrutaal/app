...
var SmoothDropLine = new Class({
	...
	start: function(){
		...
		this.tmHolderSize = $('topmenu_holder').getSize();
		
		var pb = $('topmenu_holder').getStyle('padding-bottom').toInt();
		var pt = $('topmenu_holder').getStyle('padding-top').toInt();
		this.tmHolderSize['size'].y-= pb+pt;
		...
		this.injectIn.addEvent('mouseover', function(event){														
			this.menuTree.each( function( elements, key ){
				elements.each( function(c){
					c.fx.slideOut();
					this.options.smoothFx.start({'height':this.tmHolderSize['size'].y});
...
window.addEvent('domready', function(){
	...
	var fx = new Fx.Styles('topmenu_holder', {wait:false, duration:350});
