	...
	function buildXML($params)
	{
		...
		foreach($rows as $rows_row => $rows_value){
			...
			if($rows_value->parent == 0){
				...
				//set the new level field to 0
				$rows[$rows_row]->level = 0;
...
// costta end
	...
	{	
		...
		{
			$this->_buffer .= '<ul class="subul_main dropline">';
	...
	function _getLevelXML($depth)
	{
		...
		while ($this->_current->hasChildren()){
			$this->_buffer .= '<ul class="subul_main dropline level'.$depth.'"><li class="bl"></li><li class="tl"></li><li class="tr"></li>';
			foreach ($this->_current->getChildren() as $child){
				$this->_current = & $child;
	...
	function _getItemData(&$params, $item){
		...
		if( ($yj_item_type == 1 && count($item->tree) <= 2) || ($yj_item_type == 2 && count($item->tree) <= 2) ){
			return "";
		}		
		...
		//rasio start
		//check the menu child level - $item->tree is an array with all the parent items
		if($yj_item_type == 1) {
