...
function mosGetMenuLink($mitem, $level = 0, & $params, $open = null)
{
	...
	if (!$current_itemid) {
		...
		if ($current_itemid == $mitem->id) {
			...
			if ($params->get('active_menu') && isset ($open) && in_array($mitem->id, $open)) {
				$id = 'id="active_menu2' . $params->get('class_sfx') . '"';
			} else {
				if ($mitem->type == 'url' && ItemidContained($mitem->link, $current_itemid)) {
					$id = 'id="active_menu3' . $params->get('class_sfx') . '"';
				} else {
					$id = '';
				}
			}
	...
	switch ($mitem->browserNav)
	{
		...
		case 1 :
			...
			$txt = '<a href="' . $mitem->url . '" target="_blank" class="' . $menuclass . '" ' . $id . '>' .$yj_menu_title . '</a>';
			break;

		case 2 :
			// open in a popup window
			$txt = "<a href=\"#\" onclick=\"javascript: window.open('" . $mitem->url . "', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550'); return false\" class=\"$menuclass\" " . $id . ">" . $yj_menu_title . "</a>\n";
			break;

		case 3 :
			// don't link it
			$txt = '<span class="' . $menuclass . '" ' . $id . '>' . $yj_menu_title . '</span>';
			break;

		default : // formerly case 2
			// open in parent window
			
					//$my_menu_title = $mitem->name;
	
			
			
			$txt = '<a href="' . $mitem->url . '" class="' . $menuclass . '" ' . $id . '>' . $yj_menu_title . '</a>';
			break;
	}
	...
	if ($params->get('menu_images'))
	{
		...
		if (($menu_image <> '-1') && $menu_image) {
			...
			if ($params->get('menu_images_align')) {
				//$txt = $txt . ' ' . $image;
				$image = '<img src="'.JURI::base(true).'/images/stories/' . $menu_image . '" class="imgr" border="0" alt="' . $yj_menu_title. '"/>';
				$txt = '<a href="' . $mitem->url . '" class="' . $menuclass . '" ' . $id . '>' .$yj_menu_title .$image. '</a>';
			} else {
				//$txt = $image . ' ' . $txt;
				$image = '<img src="'.JURI::base(true).'/images/stories/' . $menu_image . '" class="imgl" border="0" alt="' . $yj_menu_title. '"/>';
				$txt = '<a href="' . $mitem->url . '" class="' . $menuclass . '" ' . $id . '>' .$image. $yj_menu_title . '</a>';
			}
		}
	}
...
function mosShowVIMenu(& $params)
{
	...
	array (
			'<table width="100%" border="0" class="side_vertical" cellpadding="0" cellspacing="0">',
			'<tr ><td>',
			'</td></tr>',
			'</table>'
		),
