...
class YJmodMainMenuHelper
{
	function YJbuildXML($params)
	{
		$menu = new YJJMenuTree($params);
		$items = &JSite::getMenu();
	...
	function &YJgetXML($type, &$params, $decorator)
	{
		static $xmls;

		if (!isset($xmls[$type])) {
			$cache =& JFactory::getCache('mod_mainmenu');
			$string = $cache->call(array('YJmodMainMenuHelper', 'YJbuildXML'), $params);
			$xmls[$type] = $string;
		}
	...
	function YJrender(&$params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			...
			default :
				...
				$xml =  YJmodMainMenuHelper::YJgetXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class YJJMenuTree extends JTree
{
	/**
	...
	{
		$this->_params		=& $params;
		$this->_root		= new YJJMenuNode(0, 'ROOT');
		$this->_nodeHash[0]	=& $this->_root;
		$this->_current		=& $this->_root;
	}

	function YJaddNode(&$params, $item)
	{
		// Get menu item data
		$data = $this->YJ_getItemData($params, $item);

		// Create the node and add it
		$node = new YJJMenuNode($item->id, $item->name, $item->access, $data);
	...
	function YJ_getItemData(&$params, $item)
	{
		$data = null;
	...
	$tmp->name = '<span><![CDATA['.$item->name.']]></span>';
		...
		if ($tmp->url != null)
		{
			...
			switch ($tmp->browserNav)
			{
				...
				// _top
					$data = '<a href="'.$tmp->url.'">'.$image.'<span>'.$tmp->name.'</span></a>';
					break;
				case 1:
					// _blank
 ...
 * @since		1.5
 */
class YJJMenuNode extends JNode
{
	/**
	 * Node Title
	 */
